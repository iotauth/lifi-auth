...
To address relay attacks specifically — where an adversary forwards authentication exchanges to falsely assert physical proximity — Brands and Chaum introduced distance-bounding protocols in 1993, using round-trip signal timing to compute an upper bound on the prover-to-verifier distance~\cite{brands1993}. While foundational, these protocols remain vulnerable to mafia fraud and terrorist fraud, and critically, they bound distance without enforcing location — they answer how far but not where. A system confined by the physics of light answers the \emph{where} question directly, without dedicated timing infrastructure. It does not, however, get the \emph{how far} question for free: as discussed in Section~\ref{sec:threat-model} (G3), the present implementation has no round-trip timing check of its own, so a live optical relay that forwards genuinely fresh frames in real time is not excluded by construction. Combining the location-confinement property demonstrated here with a Brands--Chaum-style timing bound is left as future work (Section~\ref{sec:future-work}).
...
A second active research direction exploits the reciprocity and spatial variability of VLC channels to generate shared secret keys between communicating parties without prior key distribution. Zaman et al.~\cite{almoliki2017} proposed a key generation scheme for inter-vehicular VLC that extracts randomness from channel measurements, achieving key generation rates that vary with the relative positions and velocities of the vehicles. More recently, Al-Moliki et al. proposed key generation protocols for indoor VLC using OFDM subcarrier measurements, leveraging the spatial dependence of the optical channel to ensure that an eavesdropper at a different location derives a different key~\cite{keygen_ofdm2017}. Quantum key distribution over VLC channels has also been explored as an extension to post-quantum security, though practical implementations remain limited to laboratory demonstrations~\cite{qkd_vlc2025}.
...
\label{sec:key-provisioning-arch}
...
\subsubsection{Key Delivery to Receiver}

Unlike the sender, the receiver in the evaluated two-microcontroller configuration does not itself authenticate to the IoTAuth server or hold network credentials — the Pico~2~H used in this prototype has no wireless interface, and \texttt{receiver\_pico/src/main.c} implements no TLS client. Instead, the session key reaches the receiver over its USB serial console: the sender prints its active session key in hex after receiving it from the provisioning host, and an operator pastes this value into the receiver's \texttt{key <hex>} console command, which accepts the identical hex format and loads it for on-device AES-128-GCM decryption.

This is a manual, operator-mediated step in the current prototype, not an independently authenticated network exchange — the receiver's copy of the key is only as trustworthy as the local USB connection used to enter it, and it is not corroborated by any second, independent channel the way the sender's HMAC-SHA256 challenge-response (Section~\ref{sec:key-provisioning-arch}) corroborates its own. Automating this — for example, having the receiver retrieve its key from IoTAuth via a networked host companion once the receiver's optical key identifier is decoded, so both devices' keys are established over two independently authenticated channels rather than one being copied by hand — is identified as future work (Section~\ref{sec:future-work}).
...
\textbf{Definition.} A previously observed authenticated frame shall not be accepted as valid upon re-presentation to the receiver. Specifically, if a frame with nonce $n$ was accepted at time $t_1$, any subsequent frame bearing the same nonce $n$ shall be rejected for as long as $n$ remains in the receiver's replay window $W$ (Equation~\eqref{eq:replay}); window capacity bounds this guarantee, as characterized empirically in Section~\ref{sec:security-validation}.
...
\noindent where $W$ is the set of nonces currently in the replay window. Upon acceptance, $n$ is added to $W$, evicting the oldest entry if the window is full. Insertion is gated on successful GCM authentication rather than on frame-format validity alone: an earlier implementation added $n$ to $W$ once CRC and message-type checks passed, before decryption was attempted, which would have let an attacker without the session key force eviction of a genuinely captured nonce using unauthenticated, syntactically well-formed frames. This was corrected during evaluation so that only frames passing full GCM verification are recorded in $W$, ensuring eviction requires possession of the session key.
...
\subsubsection{G3 — Relay-Bounded Authorization (not achieved)}

\textbf{Definition.} A relay attack — in which the adversary captures the optical signal at the legitimate location and re-emits it at a remote location in real time — should not grant the remote receiver authorization, or should at minimum incur a detectable, bounded timing penalty proportional to the relay's introduced latency $\delta_{\text{relay}}$.

\textbf{Status: not achieved by the current implementation.} We state this explicitly rather than let it be inferred, because it is easy to conflate with G2. The freshness window $\Delta$ (G1) bounds how long a \emph{captured-and-later-resent} frame remains useful, and the nonce window (G2) rejects a \emph{byte-identical resend} outright. Neither mechanism examines \emph{when} a frame arrived relative to when it was sent, only whether its nonce is fresh and unseen. A live relay that forwards genuinely fresh frames — never storing and replaying stale bytes, simply passing each frame through with added latency $\delta_{\text{relay}}$ — produces frames that are, from the receiver's point of view, indistinguishable from frames arriving directly: the nonce has never been seen before (G2 passes), the GCM tag is valid because the ciphertext is untouched (confidentiality/integrity passes), and $t_{\text{last}}$ is continuously refreshed because relayed frames keep arriving within $\Delta$ (G1 passes). We verified this directly against the receiver implementation: the LiFi data-frame path (guarded by the nonce window in \texttt{process\_complete\_frame()}, \texttt{receiver\_pico/src/main.c}) checks only CRC validity and byte-exact nonce identity — there is no timestamp field, round-trip bound, or any other timing-derived check on the data path. Consequently, $\Delta$ does not bound relay latency at all; it bounds only the residual validity of a captured, no-longer-live frame, which is the mechanism underlying G1/G2, not G3.

\textbf{What would be required.} Relay-boundedness requires binding authorization to a measured round-trip time, not merely to nonce freshness — e.g., a distance-bounding challenge-response (Brands--Chaum style) in which the receiver issues a random nonce and the sender must respond within a tightly bounded window, with any added relay latency $\delta_{\text{relay}}$ causing the response to arrive too late to verify. This is not implemented in the current prototype; see Section~\ref{sec:discussion} and Section~\ref{sec:future-work}.

\textbf{Rationale.} Relay attacks are the most realistic threat against an optical presence system, since unlike RF, optical relay requires physical conversion (light to electrical to light) that introduces measurable delay. We report G3 as an explicit, named limitation rather than an inferred one: a system that is precise about what it does and does not defend against is more defensible at review than one that leaves the gap to be discovered.
...
\hline
G3 & Relay-Bounded Auth & \textbf{Not achieved} — no timing check on data path \\
\hline
...
The sender Pico is powered by a 12\,V DC bench supply via screw terminal; the PTN78000WAH module on the sender PCB steps this down to 5\,V for the RP2040 VSYS rail, while the four LED channels are switched directly from the 12\,V supply through the TC4420COA--SIR800DP MOSFET stage. The receiver Pico~2 is powered either via USB-C or from a single-cell LiPo battery through the BQ24074 charger and TPS63031 buck-boost on the receiver PCB. All firmware is written in C, compiled with the Pico SDK and ARM GCC toolchain. The IoTAuth key-provisioning service runs on a Raspberry Pi~4 under Linux; this host is used only at session-key setup time (Section~\ref{sec:key-provisioning-arch}) and is not part of the runtime data path between sender and receiver. Serial monitoring and control of the receiver are provided by the \texttt{rx\_monitor.py} utility.

For the freshness, revocation-latency, and replay-boundary measurements reported in Sections~\ref{sec:presence-enforcement} and~\ref{sec:security-validation}, the receiver-side protocol logic under test — CRC validation, nonce-window replay checking, and AES-128-GCM decryption — runs directly on the receiver firmware (\texttt{receiver\_pico/src/main.c}) deployed in the evaluated two-microcontroller architecture. At the time these measurements were collected, the $\Delta$-based freshness/revocation decision itself (Section~\ref{sec:presence-enforcement}) was computed by an external monitoring script observing structured per-frame event logs over USB serial, mirroring the decision logic rather than reading it from an on-device state flag. This decision logic has since been ported directly into the receiver firmware as well (\texttt{presence\_check\_decay()}), making revocation a purely on-device determination; Section~\ref{sec:presence-enforcement} reports a third measurement batch collected directly from this on-device implementation, confirming the earlier externally-measured numbers.
...
Replay protection is enforced by the 64-entry nonce sliding window (\texttt{NONCE\_HISTORY\_SIZE}). Each incoming frame's 12-byte nonce is checked against the window before decryption is attempted. A frame bearing any nonce already present in the window is rejected without invoking the GCM primitive. Nonce uniqueness within a boot session is guaranteed by the monotonically increasing 32-bit counter field; uniqueness across reboots is ensured with overwhelming probability by the 8-byte random boot salt generated from the RP2040 hardware ring-oscillator entropy source via CTR\_DRBG at startup. Counter exhaustion ($2^{32}$ messages) triggers a device reboot to regenerate the salt, preventing nonce reuse.

\textbf{Replay rejection test.} Using an automated harness (\texttt{thesis\_eval/replay\_test.py}) that captures one valid encrypted frame's raw bytes (type, nonce, ciphertext, and GCM tag) directly off the receiver's decoded output and re-injects the identical bytes back into the receiver's input, we ran 25 independent trials. In all 25/25 trials the replayed frame was rejected immediately, with the nonce-window check (\texttt{replay=YES}) firing before decryption was attempted, confirming G2 as designed with no observed exceptions.

\textbf{Replay window boundary test.} The same harness then reduced the nonce window's capacity to 2 entries at runtime, allowed 2 further legitimate frames to be received (evicting the captured frame's nonce from the window), and re-injected the identical captured frame a second time, across the same 25 trials. In all 25/25 trials the replayed frame was accepted and successfully decrypted, with no exceptions. This reproduces a genuine, consistently reproducible boundary limitation of the sliding-window design: replay protection is bounded by window capacity, not permanent — once a nonce has aged out of the window (64 entries in the production configuration, versus the 2-entry setting used here to make the boundary condition observable within a short test), an identical previously-accepted frame becomes acceptable again. In practice, exhausting the production 64-slot window before an adversary reuses a captured frame requires 64 further legitimate frames to be exchanged, which at the sender's minimum authenticated-frame rate ($1/\Delta$, see Section~\ref{sec:presence-enforcement}) bounds how long a captured frame remains a live replay threat — but the limitation is real and is not eliminated by window size alone. This distinction is recorded here explicitly rather than left for a reviewer to discover.

Quantitative measurements of tag verification latency and tampered-frame rejection rates across a larger packet sample remain future work.

\subsection{Presence Enforcement}
\label{sec:presence-enforcement}

Authorization revocation upon optical interruption follows directly from the freshness mechanism defined in Equation~\eqref{eq:freshness}. When the optical path is broken, no new authenticated frames reach the receiver, and the authorization timestamp $t_{\text{last}}$ becomes stale. Within at most $\Delta$ time units, the freshness condition evaluates to false and the receiver revokes authorization without any explicit network-level revocation message.

\textbf{Occlusion (shadowing) test.} The production configuration uses $\Delta = 15\,\text{s}$ (\texttt{LIVENESS\_WINDOW\_MS} in the receiver firmware's \texttt{presence\_check\_decay()}; mirrored by an external monitoring script at the time these measurements were collected, see Section~\ref{sec:experimental-setup}). To exercise this experimentally, the sender firmware was extended with an idle-time heartbeat — a small authenticated frame sent automatically every 2.5\,s using the existing encrypt/frame/send path — so that continuous traffic exists to interrupt. With the link at steady-state (\texttt{verified: true}), the optical path was physically occluded at a marked $t=0$; the time to the corresponding revocation event was logged, the path was then unblocked, and the time back to re-verification was logged. This was repeated across three batches: 21 trials polling receiver state at 5.0\,s intervals (the external monitoring script's production polling cadence), 20 trials at 0.2\,s intervals (to isolate polling-granularity jitter from the mechanism's intrinsic latency), and, following the on-device port of the revocation decision (\texttt{presence\_check\_decay()}), 25 trials reading the receiver's own \texttt{[PRESENCE]} state transitions directly, with no external polling involved at all.

\begin{table}[htbp]
\caption{Revocation and Re-verification Latency ($\Delta = 15\,\text{s}$)}
\begin{center}
\small
\begin{tabular}{|l|c|c|}
\hline
\textbf{Condition} & \textbf{Time-to-revoke} & \textbf{Time-to-reverify} \\
\hline
As-deployed (5\,s poll), $n{=}21$ & 17.27\,s $\pm$ 1.25\,s & 1.43\,s $\pm$ 0.69\,s \\
\hline
Fine-grained (0.2\,s poll), $n{=}20$ & 15.11\,s $\pm$ 0.06\,s & 1.30\,s $\pm$ 0.65\,s \\
\hline
On-device decision, $n{=}25$ & 15.001\,s $\pm$ 0.000\,s & 1.33\,s $\pm$ 0.62\,s \\
\hline
\end{tabular}
\label{tab:revocation_latency}
\end{center}
\end{table}

At the production polling cadence, measured time-to-revoke was bimodal (clustering near 15.7\,s and 18.2\,s), an artifact of the 5\,s poll interval aliasing against the underlying revocation event rather than a property of the freshness mechanism itself. At the fine-grained polling cadence, time-to-revoke collapsed to a tight cluster at $15.11\,\text{s} \pm 0.06\,\text{s}$ — essentially exactly $\Delta$ plus a fixed $\approx 0.11\,\text{s}$ processing overhead — confirming that the mechanism enforces its configured bound precisely, and that the additional latency and variance observed in the as-deployed measurement is attributable to the external monitoring script's poll granularity, not the underlying protocol. Reading the revocation decision directly from the receiver's own \texttt{[PRESENCE]} transitions confirms this without relying on any external observation cadence at all: time-to-revoke collapsed further, to $15.001\,\text{s} \pm 0.000\,\text{s}$ across all 25 trials — every trial hit the same value because the on-device check runs every main-loop iteration, so revocation fires the instant the timer crosses $\Delta$, independent of how long the occlusion continues afterward. This closes the on-device re-confirmation previously identified as pending work (Section~\ref{sec:limitations}) and is the strongest available evidence that $\Delta$ is enforced exactly, not just approximately. Time-to-reverify was consistent across all three batches ($\approx 1.3$--$1.4\,\text{s}$) and is bounded above by the 2.5\,s heartbeat interval, as expected.

\textbf{$\Delta$ sensitivity to benign traffic.} To assess whether $\Delta = 15\,\text{s}$ is justified rather than arbitrary, we logged inter-frame gaps over a 40-minute benign session (960 frames, 959 gaps) and computed, for candidate $\Delta \in \{3, 5, 10, 15, 20, 30\}\,\text{s}$, the fraction of observed gaps that would have exceeded each candidate (a false-revoke event under benign conditions). With heartbeat-only traffic, gaps clustered tightly at 2.409--2.591\,s (98\% within $\pm 20\,\text{ms}$ of the 2.5\,s heartbeat interval), yielding a measured false-revoke rate of 0.0\% at every candidate $\Delta$ tested, including the smallest. This result is degenerate rather than informative: a single mechanically regular heartbeat does not exhibit the gap variability that real, possibly degraded, optical conditions would produce, so it cannot by itself justify a specific $\Delta$ against false-revoke risk. We therefore report the finding narrowly — $\Delta = 15\,\text{s}$ carries zero measured false-revoke cost against heartbeat-only traffic — and identify a full parametric justification of $\Delta$, characterizing gap variability under real degraded-signal conditions (partial occlusion, marginal alignment, ambient light transients), as future work.

At a reliable frame rate of up to 300\,kbaud with standard message sizes, the sender is capable of delivering many authenticated frames per $\Delta$ interval, well in excess of the minimum rate $r > 1/\Delta$ required to maintain continuous authorization for a legitimately positioned receiver.
...
\subsection{Relay Attack Resilience — a Named Limitation}

As established in Section~\ref{sec:threat-model} (G3), the current implementation does \emph{not} bound relay attacks. The $\Delta$-bounded freshness mechanism (G1) and the nonce sliding window (G2) together prevent an adversary from presenting a \emph{captured, no-longer-live} frame after the fact, but neither examines transit time, so a \emph{live} relay that forwards genuinely fresh frames in real time — introducing latency $\delta_{\text{relay}}$ but never storing and resending stale bytes — passes every check the receiver performs. This distinction matters because it is easy to mistake G1's expiration window for a relay bound; it is not, since a continuously operating relay keeps $t_{\text{last}}$ refreshed indefinitely.

Closing this gap would require binding authorization to a measured round-trip time rather than to nonce freshness alone: a distance-bounding challenge-response (Brands--Chaum style), in which the receiver periodically issues a random nonce and the sender must respond within a tightly bounded window, such that any relay-introduced latency $\delta_{\text{relay}}$ becomes directly detectable. Since the optical channel operates at the speed of light (negligible propagation delay at room scale) and the existing bidirectional UART link between sender and receiver already provides the necessary transport, this extension is architecturally compatible with the current design but is not implemented in the current prototype. As a data point supporting its feasibility, not as a security guarantee in itself: end-to-end frame processing latency on the receiver firmware was measured at 3--6\,ms (preamble lock to full CRC-validated frame), suggesting there is ample timing margin to add a bounded round-trip check without a hardware redesign — but doing so, and validating it, remains future work (Section~\ref{sec:future-work}).
...
\item \textbf{No relay resistance (G3 not achieved).} As detailed in Section~\ref{sec:threat-model} and Section~\ref{sec:discussion}, the current implementation enforces freshness (G1) and replay resistance (G2) but performs no round-trip timing check on the optical data path. A live relay that forwards genuinely fresh frames in real time, rather than resending captured stale bytes, would pass every check the receiver currently performs. Achieving relay-boundedness would require a distance-bounding challenge-response extension, which is not yet implemented.
...
\item \textbf{Receiver key delivery is manual, not independently authenticated.} Unlike the sender, which acquires its session key over UART with HMAC-SHA256 challenge-response verification (Section~\ref{sec:key-provisioning-arch}), the receiver's copy is entered by an operator pasting hex output from the sender's console into the receiver's own console. This is adequate for a benchtop prototype but is not corroborated by a second, independent channel the way the sender's key is — an operator error or a compromised console session could load an incorrect or stale key without detection.
...
\item \textbf{Independently authenticated receiver key delivery.} Automating receiver key acquisition — e.g., a networked host companion that retrieves the session key from IoTAuth over TLS once the receiver has decoded a key identifier from the sender, mirroring the sender's own provisioning path — would replace the current manual console-paste step with a second, independently authenticated channel, removing the operator-trust dependency identified in Section~\ref{sec:limitations}.
...
By formalizing the security goals of freshness, replay resistance, relay-bounded authorization, and revocation on interruption, and by demonstrating enforcement of freshness, replay resistance, and interruption-revocation through a physical prototype — while identifying relay-boundedness as an explicit, unimplemented goal for future work — this work establishes that light itself --- confined by physics, modulated by electronics, and authenticated by cryptography --- can serve as a verifiable and continuously enforced proof of physical presence.
...
\bibitem{gomez2015}
A. Gomez, K. Shi, C. Quintana, M. Sato, G. Faulkner, B. C. Thomsen, and D. C. O'Brien, ``Beyond 100-Gb/s Indoor Wide Field-of-View Optical Wireless Communications,'' \textit{IEEE Photonics Technology Letters}, vol. 27, no. 4, pp. 367--370, Feb. 2015.
...
G. Blinowski, ``Security of Visible Light Communication systems --- A survey,'' \textit{Physical Communication}, vol. 34, pp. 246--260, June 2019.
...
\bibitem{irs_vlc2024}
M. Caputo, L. Mucchi, and L. Pierucci, ``Multi-RIS Aided VLC Physical Layer Security for 6G Wireless Networks,'' \textit{IEEE Transactions on Mobile Computing}, 2024.

\bibitem{almoliki2017}
I. U. Zaman, A. B. Lopez, M. A. Al Faruque, and O. Boyraz, ``A Physical Layer Security Key Generation Technique for Inter-Vehicular Visible Light Communication,'' in \textit{Advanced Photonics Congress (IPR, Networks, NOMA, PS, SPPCom)}, OSA Technical Digest, paper SpTu1F.3, 2017.

\bibitem{keygen_ofdm2017}
Y. M. Al-Moliki, M. T. Alresheedi, and Y. Al-Harthi, ``Secret Key Generation Protocol for Optical OFDM Systems in Indoor VLC Networks,'' \textit{IEEE Photonics Journal}, vol. 9, no. 2, pp. 1--15, Apr. 2017.
...
\bibitem{lisa2019}
T. Perkovi\'{c}, M. \v{C}agalj, and T. Kova\v{c}evi\'{c}, ``LISA: Visible light based initialization and SMS based authentication of constrained IoT devices,'' \textit{Future Generation Computer Systems}, vol. 97, pp. 775--791, Aug. 2019.
