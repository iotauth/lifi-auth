...
\textbf{Replay rejection test.} We captured one valid encrypted frame's raw bytes (type, nonce, ciphertext, and GCM tag) directly off the receiver's decoded output and re-injected the identical bytes back into the receiver's input. The replayed frame was rejected immediately, with the nonce-window check (\texttt{replay=YES}) firing before decryption was attempted, confirming G2 as designed.

\textbf{Replay window boundary test.} We then reduced the nonce window's capacity to 2 entries at runtime, allowed 2 further legitimate frames to be received (evicting the captured frame's nonce from the window), and re-injected the identical captured frame a second time. In this case the replayed frame was accepted and successfully decrypted. This reproduces a genuine boundary limitation of the sliding-window design: replay protection is bounded by window capacity, not permanent — once a nonce has aged out of the window (64 entries in the production configuration, versus the 2-entry setting used here to make the boundary condition observable within a short test), an identical previously-accepted frame becomes acceptable again. In practice, exhausting the production 64-slot window before an adversary reuses a captured frame requires 64 further legitimate frames to be exchanged, which at the sender's minimum authenticated-frame rate ($1/\Delta$, see Section~\ref{sec:presence-enforcement}) bounds how long a captured frame remains a live replay threat — but the limitation is real and is not eliminated by window size alone. This distinction is recorded here explicitly rather than left for a reviewer to discover.
