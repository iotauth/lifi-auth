...
For the freshness, revocation-latency, and replay-boundary measurements reported in Sections~\ref{sec:presence-enforcement} and~\ref{sec:security-validation}, the receiver-side protocol logic under test was exercised on a debug receiver (\texttt{receiver\_pico/src/main.c}) rather than the full Pi~4 deployment path (\texttt{dash\_receiver.c}). The two implementations parse an identical wire format and share the same freshness state machine and 64-entry nonce sliding window (\texttt{NONCE\_HISTORY\_SIZE}), differing only in that the debug receiver omits the WiFi/HMAC control-plane layer used to report status to the dashboard, which simplified direct instrumentation of protocol-level events. Confirming these measurements against the production \texttt{dash\_receiver.c}/Pi~4 path is identified as pending work in Section~\ref{sec:limitations}.
...
\textbf{Occlusion (shadowing) test.} The production configuration uses $\Delta = 15\,\text{s}$ (\texttt{LIVENESS\_WINDOW\_S} in the dashboard's \texttt{pi4\_health\_monitor()}). To exercise this experimentally, the sender firmware was extended with an idle-time heartbeat — a small authenticated frame sent automatically every 2.5\,s using the existing encrypt/frame/send path — so that continuous traffic exists to interrupt. With the link at steady-state (\texttt{verified: true}), the optical path was physically occluded at a marked $t=0$; the time to the corresponding revocation event was logged, the path was then unblocked, and the time back to re-verification was logged. This was repeated across two batches: 21 trials polling receiver state at 5.0\,s intervals (the dashboard's production polling cadence) and 20 trials at 0.2\,s intervals (to isolate polling-granularity jitter from the mechanism's intrinsic latency).
...
\item \textbf{Revocation-latency and replay measurements pending confirmation on the production receiver.} The quantitative results in Section~\ref{sec:presence-enforcement} and Section~\ref{sec:security-validation} were measured against a debug receiver that shares identical wire-protocol parsing and freshness/replay logic with the production Pi~4 receiver (\texttt{dash\_receiver.c}), but omits its WiFi/HMAC control-plane layer. Repeating these measurements against the full Pi~4 deployment is planned to confirm the reported numbers carry over unchanged.
\end{enumerate}
