...
\textbf{Occlusion (shadowing) test.} The production configuration uses $\Delta = 15\,\text{s}$ (\texttt{LIVENESS\_WINDOW\_MS} in the receiver firmware's \texttt{presence\_check\_decay()}; mirrored by an external monitoring script at the time these measurements were collected, see Section~\ref{sec:experimental-setup}). To exercise this experimentally, the sender firmware was extended with an idle-time heartbeat — a small authenticated frame sent automatically every 2.5\,s using the existing encrypt/frame/send path — so that continuous traffic exists to interrupt. With the link at steady-state (\texttt{verified: true}), the optical path was physically occluded at a marked $t=0$; the time to the corresponding revocation event was logged, the path was then unblocked, and the time back to re-verification was logged. This was repeated across two batches: 21 trials polling receiver state at 5.0\,s intervals (the external monitoring script's production polling cadence) and 20 trials at 0.2\,s intervals (to isolate polling-granularity jitter from the mechanism's intrinsic latency).
...
\item \textbf{Revocation-latency measurements pending re-confirmation against the on-device decision.} The quantitative freshness/revocation results in Section~\ref{sec:presence-enforcement} were collected while the $\Delta$-based revocation decision was computed by an external monitoring script observing the receiver's per-frame event log, rather than by the receiver's own on-device state. This decision logic has since been ported directly into the receiver firmware (\texttt{presence\_check\_decay()} in \texttt{receiver\_pico/src/main.c}); repeating the occlusion measurements against this on-device implementation to confirm the reported numbers carry over unchanged is planned but not yet completed.
\end{enumerate}
